...
patterns = ["**/*.mid", "**/*.midi"]
paths_for_pattern = (
    lambda pattern: Path("./data/maestro-v3.0.0").resolve().glob(pattern)
)
midi_paths = [path for pattern in patterns for path in paths_for_pattern(pattern)]

bp_left, bp_right = int(len(midi_paths) * train_proportion), int(
    len(midi_paths) * (train_proportion + dev_proportion)
)
